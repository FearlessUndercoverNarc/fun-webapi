...
namespace Services.Versioned.Implementations
{
    public partial class DeskShareService : IDeskShareServiceV1
    {
        ...
        public DeskShareService(IFolderRepository folderRepository, IDeskRepository deskRepository, IDeskShareRepository deskShareRepository, IRequestAccountIdService requestAccountIdService, IFolderShareRepository folderShareRepository, IMapper mapper)
        {
        ...
        async Task IDeskShareServiceV1.Share(long id, long recipientId, bool hasWriteAccess)
        {
        ...
        async Task<ICollection<DeskShareDto>> IDeskShareServiceV1.GetShares(long id)
        {
            var requestAccountId = _requestAccountIdService.Id;
            var desk = await _deskRepository.GetById(id);
            ...
            if (desk.AuthorAccountId != requestAccountId)
            {
                await TelegramAPI.Send($"IDeskShareServiceV1.GetShares:\nAttempt to access restricted desk!\nDeskId ({id})\nUser ({_requestAccountIdService.Id})");
